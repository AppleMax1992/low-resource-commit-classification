...
from .smooth_loss import CumulativeSmoothLoss
...
class MoEModel(nn.Module):
    ...
    def trainer(self, train_loader, val_loader, num_epochs=3, learning_rate=2e-5, patience=3):
        ...
        cfl = CumulativeSmoothLoss(smoothing_start=0.2, smoothing_end=0.0, total_epochs=num_epochs)
        ...
        for epoch in range(num_epochs):
            ...
            with tqdm(total=len(train_loader), desc=f'Epoch {epoch+1}/{num_epochs}', unit='batch') as pbar:
                for batch in train_loader:
                    ...
                    loss = cfl.smooth_loss(logits, labels, epoch + 1)
                    loss.backward()
                    optimizer.step()
